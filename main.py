...
def safe_api(url, params=None, retries=3):
    """具備重試機制的 API 請求，精確處理 403 與 422"""
    for i in range(retries):
        try:
            r = requests.get(url, params=params, timeout=25)
            if r.status_code == 200:
                return r.json()
            log(f"⚠️ API 狀態碼 {r.status_code}: {url.split('?')[0]}")
...
# =========================
# 擷取層 (Crawlers)：官方文件精確對齊版
# =========================
def harvest_ultimate(ticker, is_tw=True):
    log(f"🚀 開始收割: {ticker} ({'台股' if is_tw else '美股'})")
    ...
    # 1. 財務報表 (對齊 FMP v3 官方 Path Parameter 格式)
    fmp_v3 = "https://financialmodelingprep.com/api/v3"
    log(f"📊 下載 {ticker} 四大報表與權益變動表...")
    ...
    # 3. FinMind 在地數據 (僅針對台股，使用探針成功的 2026 參數)
    fm_res = {}
    if is_tw and fm_token:
        fm_url = "https://api.finmindtrade.com/api/v4/data"
        # 僅抓取探針已證實成功的營收、籌碼、可轉債
        ...
        for ds in datasets:
            data = safe_api(fm_url, {
                ...
                "start_date": "2026-01-01" # 鎖定探針成功的日期
            })
    ...
    payload = {
        ...
        "intelligence": {
            ...
            "info_snapshot": stock.info if hasattr(stock, 'info') else {}
        }
    }
